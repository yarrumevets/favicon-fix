...
Registers the `/favicon.ico` route on the provided Express-compatible app and serves the built-in red favicon.

- **app** — any object with `get()` or `use()` + `send()` semantics (Express, Fastify-express, etc.).
